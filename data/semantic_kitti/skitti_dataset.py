...
class KittiDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        scale_3ds = [self.output_scale, self.project_scale, 4, 8, 16, 32]
        data["scale_3ds"] = scale_3ds
        cam_k = P[0:3, 0:3]
        data["cam_k"] = cam_k
        vox2pix_path = os.path.join(self.root, "dataset", "sequences", sequence, "vox2pix", frame_id + ".npy")
        ...
        if self.split != "test":
            depth_path = os.path.join(self.root, "dataset", "sequences", sequence, "depth_map", frame_id + ".npy")
            sem_path = os.path.join(self.root, "dataset", "sequences", sequence, "sem_map", frame_id + ".npy")
            data["proj_label"] = np.load(sem_path).astype(np.int32)
            data["proj_depth"] = np.load(depth_path).astype(np.float32)
        else:
            data["proj_label"] = None
            data["proj_depth"] = None
        # Image augmentation
        ...
        #data["is_flip"] = (np.random.rand() < self.fliplr)
